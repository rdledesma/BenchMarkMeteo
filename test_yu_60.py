...
data = pd.read_csv('/home/inenco/Documentos/Medidas/yu_15.csv')
data['datetime'] = pd.to_datetime(data.datetime)

data['ghi'] = np.where(
    (data.datetime.dt.date >= date(2018, 1, 5)) &
    (data.datetime.dt.date <= date(2018, 1, 31)),
    np.nan,
    data.ghi
)
...
data['ghi'] = np.where(
    (data.datetime.dt.date >= date(2018, 2, 17)) &
    (data.datetime.dt.date <= date(2018, 3, 1)),
    np.nan,
    data.ghi
)

data['ghi'] = np.where(
    (data.datetime.dt.date >= date(2018, 4, 9)) &
    (data.datetime.dt.date <= date(2018, 5, 1)),
    np.nan,
    data.ghi
)
...
data['ghi'] = np.where(
    (data.datetime.dt.date >= date(2018, 10, 23)) &
    (data.datetime.dt.date <= date(2018, 11, 1)),
    np.nan,
    data.ghi
)
...
data = (data.set_index('datetime')
    ...
    start="2017/01/01 00:00", end="2018/12/31 23:59", freq="15min"))
...
cams = pd.read_csv('/home/inenco/Documentos/HELIOSAT/yu.csv', usecols=['GHI'], sep=";", 
                   header=42)
...
lsasaf = pd.read_csv('/home/inenco/Documentos/LSASAF/yu_15.csv')
...
Metrics.rrmsd(X.ghi, X.era)
Metrics.rrmsd(X.ghi, X.merra)
