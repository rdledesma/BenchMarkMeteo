...
#X.to_csv('pil15.csv', index=False)
...
# Convertir yearly a formato largo
yearly_long = yearly.reset_index().melt(id_vars="year", 
                                        var_name="model", 
                                        value_name="rrmsd")

# Calcular media y std por modelo
yearly_stats = yearly_long.groupby("model")["rrmsd"].agg(["mean","std"])

# Plot con barras de error
ax = yearly_stats["mean"].plot(kind='bar', yerr=yearly_stats["std"], 
                               capsize=5, figsize=(10,5))
plt.ylabel("rRMSE (%)")
plt.title(f"Yearly rRMSE mean ± std - {site.cod}")
plt.show()


# Convertir seasonal a formato largo
seasonal_long = seasonal.reset_index().melt(id_vars="season", 
                                            var_name="model", 
                                            value_name="rrmsd")

# Calcular media y std por modelo
seasonal_stats = seasonal_long.groupby("model")["rrmsd"].agg(["mean","std"])

# Plot con barras de error
ax = seasonal_stats["mean"].plot(kind='bar', yerr=seasonal_stats["std"], 
                                 capsize=5, figsize=(8,5))
plt.ylabel("rRMSE (%)")
plt.title(f"Seasonal rRMSE mean ± std - {site.cod}")
plt.show()
